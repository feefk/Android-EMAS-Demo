...
modifyGradleProperties() {
    ...
    if [ "$CUSTOM_REPOSITORY_HOST" != "" ]; then
        sed -i "s/CUSTOM_REPOSITORY_HOST =.*/CUSTOM_REPOSITORY_HOST = $CUSTOM_REPOSITORY_HOST/g" gradle.properties
    fi
    ...
    if [ "$CUSTOM_REPOSITORY_HOST" != "" ]; then
        sed -i "s/SDK_REPOSITORY_URL =.*/SDK_REPOSITORY_URL = $CUSTOM_REPOSITORY_HOST\/repository\/maven-public\//g" gradle.properties
    fi

    if [ "$CUSTOM_REPOSITORY_USERNAME" != "" ]; then
        sed -i "s/SDK_REPOSITORY_USERNAME =.*/SDK_REPOSITORY_USERNAME = $CUSTOM_REPOSITORY_USERNAME/g" gradle.properties
    fi

    if [ "$CUSTOM_REPOSITORY_PASSWORD" != "" ]; then
        sed -i "s/SDK_REPOSITORY_PASSWORD =.*/SDK_REPOSITORY_PASSWORD = $CUSTOM_REPOSITORY_PASSWORD/g" gradle.properties
    fi

    if [ "$CUSTOM_REPOSITORY_HOST" != "" ]; then
        sed -i "s/WEEX_PUBLIC_REPOSITORY_URL =.*/WEEX_PUBLIC_REPOSITORY_URL = $CUSTOM_REPOSITORY_HOST\/repository\/weex-public\//g" gradle.properties
    fi

    if [ "$CUSTOM_REPOSITORY_USERNAME" != "" ]; then
        sed -i "s/WEEX_REPOSITORY_USERNAME =.*/WEEX_REPOSITORY_USERNAME = $CUSTOM_REPOSITORY_USERNAME/g" gradle.properties
    fi

    if [ "$CUSTOM_REPOSITORY_PASSWORD" != "" ]; then
        sed -i "s/WEEX_REPOSITORY_PASSWORD =.*/WEEX_REPOSITORY_PASSWORD = $CUSTOM_REPOSITORY_PASSWORD/g" gradle.properties
    fi
...
modifyGradleWrapperProperties() {
    ...
    if [ "$CUSTOM_REPOSITORY_HOST" != "" ]; then
        # 只替换maven域名
        sed -i "s/^distributionUrl=.*\(\/repository\/.*\)/distributionUrl=$CUSTOM_REPOSITORY_HOST\1/g" ./gradle/wrapper/gradle-wrapper.properties
    fi
...
modifyNativeSDk() {
    ...
    if [ "$SDK_CONFIG_API_DOMAIN" != "" ]; then
        sed -i "s/\"APIDomain\".*/\"APIDomain\": \"$SDK_CONFIG_API_DOMAIN\"/g" $SDK_PATH
    fi
...
modifyScaffoldType() {
    ...
    if [ "$SCAFFOLD_TYPE" != "" ]; then
        sed -i "s/\"ScaffoldType\": [0-9]/\"ScaffoldType\": $SCAFFOLD_TYPE/g" app/src/main/assets/weex-container.json
    fi
    echo "done"
}
...
modifyHAUploadType() {
    echo "start modify HA upload type"
    if [ "$HA_UPLOAD_TYPE" != "" ]; then
        #打开ceph依赖，关闭oss依赖
        sed -i "/com.taobao.android:tlog_uploader_ceph/{ s/\///g }" $GRADLE_PATH
        sed -i "/tlog_uploader_oss/{ s/\///g }" $GRADLE_PATH
        sed -i "/com.aliyun.dpa:oss-android-sdk/{ s/compile/\/\/compile/g }" $GRADLE_PATH
    fi
    echo "done"
}
